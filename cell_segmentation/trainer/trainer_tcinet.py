...
#
# 改进：validation_epoch 返回 mPQ 作为 best model 保存标准（原版返回 bPQ）
...
class TCINetTrainer(BaseTrainer):
    ...
    def train_step(self, batch, batch_idx, num_batches, return_example_images):
    ...
    def validation_epoch(self, epoch, val_dataloader):
        self.model.eval()
        ...
        mpq_score = np.nanmean([np.nanmean(pq) for pq in cell_type_pq_scores])
        ...
        scalar_metrics = {
            ...
            "mPQ/Validation": mpq_score,
        }
        for branch, loss_fns in self.loss_fn_dict.items():
            for loss_name in loss_fns:
                scalar_metrics[f"{branch}_{loss_name}/Validation"] = self.loss_avg_tracker[
                    f"{branch}_{loss_name}"
                ].avg
        ...
        # ===== 双指标保存：mPQ 和 bPQ =====
        bpq_score = np.nanmean(pq_scores)

        if not hasattr(self, "_best_mpq"):
            self._best_mpq = -1.0
        if mpq_score > self._best_mpq:
            self._best_mpq = mpq_score
            self.save_checkpoint(epoch, "model_best_mpq.pth")
            self.logger.info(f"New best mPQ: {mpq_score:.4f} → model_best_mpq.pth")

        if not hasattr(self, "_best_bpq"):
            self._best_bpq = -1.0
        if bpq_score > self._best_bpq:
            self._best_bpq = bpq_score
            self.save_checkpoint(epoch, "model_best_bpq.pth")
            self.logger.info(f"New best bPQ: {bpq_score:.4f} → model_best_bpq.pth")
        # =====================================

        return scalar_metrics, image_metrics, mpq_score

    def validation_step(self, batch, batch_idx, return_example_images):
        imgs = batch[0].to(self.device)
        masks = batch[1]
        tissue_types = batch[2]
    ...
    def unpack_masks(self, masks: dict, tissue_types: list) -> DataclassHVStorage:
        ...
        # FIX: handle last batch with single image
        if gt_nuclei_type_maps_onehot.dim() == 3:
            gt_nuclei_type_maps_onehot = gt_nuclei_type_maps_onehot.unsqueeze(0)
        if gt_nuclei_binary_map_onehot.dim() == 3:
            gt_nuclei_binary_map_onehot = gt_nuclei_binary_map_onehot.unsqueeze(0)
    ...
    def calculate_loss(self, predictions, gt) -> torch.Tensor:
    ...
    def calculate_step_metric_train(self, predictions, gt) -> dict:
        predictions = predictions.get_dict()
        gt = gt.get_dict()
        ...
        for i in range(len(pred_tissue)):
            ...
            cell_jaccard = binary_jaccard_index(
                preds=pred_binary_map, target=target_binary_map
            ).detach().cpu()
    ...
    def calculate_step_metric_validation(self, predictions, gt) -> dict:
        predictions = predictions.get_dict()
        gt = gt.get_dict()
    ...
    @staticmethod
    def generate_example_image(imgs, predictions, gt, num_nuclei_classes, num_images=2):
        predictions = predictions.get_dict()
        gt = gt.get_dict()

        # FIX: adapt to actual batch size
        ...
        pred_sample_type_maps = torch.argmax(
            predictions["nuclei_type_map"][sample_indices], dim=-1
        ).detach().cpu().numpy()
        ...
        gt_sample_type_map = torch.argmax(
            gt["nuclei_type_map"][sample_indices], dim=-1
        ).detach().cpu().numpy()

        hv_cmap = plt.get_cmap("jet")
        binary_cmap = plt.get_cmap("jet")
        instance_cmap = plt.get_cmap("viridis")
        ...
        for i in range(num_images):
            ...
            placeholder[:h, :w] = sample_images[i]
            placeholder[h:2*h, :w] = sample_images[i]
            placeholder[:h, w:2*w] = rgba2rgb(binary_cmap(gt_sample_binary_map[i] * 255))
            placeholder[h:2*h, w:2*w] = rgba2rgb(binary_cmap(pred_sample_binary_map[i]))
            placeholder[:h, 2*w:3*w] = rgba2rgb(hv_cmap((gt_sample_hv_map[i, :, :, 0] + 1) / 2))
            placeholder[h:2*h, 2*w:3*w] = rgba2rgb(hv_cmap((pred_sample_hv_map[i, :, :, 0] + 1) / 2))
            placeholder[:h, 3*w:4*w] = rgba2rgb(hv_cmap((gt_sample_hv_map[i, :, :, 1] + 1) / 2))
            placeholder[h:2*h, 3*w:4*w] = rgba2rgb(hv_cmap((pred_sample_hv_map[i, :, :, 1] + 1) / 2))
            placeholder[:h, 4*w:5*w] = rgba2rgb(
                instance_cmap(
                    (gt_sample_instance_map[i] - np.min(gt_sample_instance_map[i]))
                    / (np.max(gt_sample_instance_map[i]) - np.min(gt_sample_instance_map[i] + 1e-10))
                )
            )
            placeholder[h:2*h, 4*w:5*w] = rgba2rgb(
                instance_cmap(
                    (pred_sample_instance_maps[i] - np.min(pred_sample_instance_maps[i]))
                    / (np.max(pred_sample_instance_maps[i]) - np.min(pred_sample_instance_maps[i] + 1e-10))
                )
            )
            placeholder[:h, 5*w:6*w] = rgba2rgb(binary_cmap(gt_sample_type_map[i] / num_nuclei_classes))
            placeholder[h:2*h, 5*w:6*w] = rgba2rgb(binary_cmap(pred_sample_type_maps[i] / num_nuclei_classes))
            ...
            for x_seg in np.arange(w, 6 * w, w):
                axs[i].axvline(x_seg, color="black")
            for y_seg in np.arange(h, 2 * h, h):
