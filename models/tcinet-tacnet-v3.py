...
class ChannelMixer(nn.Module):
    """对应原文 Figure 3 LKCell Block 下半部分的通道混合模块"""
    ...
    def forward(self, x):
        return x + self.net(x)  # 独立的残差连接


class SpatialMixer(nn.Module):
    """对应原文 Figure 3 LKCell Block 上半部分的空间混合模块 (大核+并行小核)"""
    def __init__(self, channels: int, large_kernel_size: int = 13, small_kernel_size: int = 5, dilation: int = 1):
        super().__init__()
        
        # 前置 1x1 瓶颈层 (对应图中的 BN -> Conv 1x1 -> ReLU)
        ...
        # 并行分支 1：大卷积核 (DW KxK, BN)
        padding_large = large_kernel_size // 2
        self.large_conv = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=large_kernel_size, 
                      padding=padding_large, groups=channels, bias=False),
            nn.BatchNorm2d(channels)
        )
        
        # 并行分支 2：小卷积核 / 空洞卷积 (Conv kxk)
        padding_small = (small_kernel_size + (small_kernel_size - 1) * (dilation - 1)) // 2
        self.small_conv = nn.Conv2d(channels, channels, kernel_size=small_kernel_size, 
                                    padding=padding_small, dilation=dilation, groups=channels, bias=False)
    ...
    def forward(self, x):
        ...
        # 1. 瓶颈层特征变换
        feat = self.pre_proj(x)
        
        # 2. 核心：并行执行大核与小核，并相加融合多尺度特征 (你的原代码里写成了串行)
        feat_large = self.large_conv(feat)
        feat_small = self.small_conv(feat)
        feat = feat_large + feat_small  
...
class LKCellBlock(nn.Module):
    """
    完美的 LKCell Block 实现，严格对齐论文 Figure 3。
    包含底部的 Channel Mixer 和顶部的 Spatial Mixer。
    """
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 13):
        ...
        self.channel_mixer = ChannelMixer(out_channels)
        self.spatial_mixer = SpatialMixer(out_channels, large_kernel_size=kernel_size)

    def forward(self, x):
        x = self.channel_align(x)
        x = self.channel_mixer(x)  # 先进行特征维度的混合
        x = self.spatial_mixer(x)  # 再进行大感受野的空间混合
        return x
