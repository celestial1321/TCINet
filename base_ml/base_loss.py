...
class BoundaryWeightedBCELoss(nn.Module):
    ...
    从 GT binary mask 提取边界区域（膨胀-腐蚀），对边界像素加大 loss 权重。
    论文支撑：MC-NuSeg (IPMI 2025), BRP-Net (MICCAI 2020)
    Args:
        boundary_weight: 边界像素权重倍数，默认 5.0
        kernel_size: 边界提取核大小，默认 3
    """
    ...
    def _extract_boundary(self, binary_mask: torch.Tensor) -> torch.Tensor:
...
LOSS_DICT = {
    ...
    "BoundaryWeightedBCELoss": BoundaryWeightedBCELoss,
}
